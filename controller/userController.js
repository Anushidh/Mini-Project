...
const userProfile = async (req, res) => {
  try {
    ...
    // console.log(userOrders,'userrrrrrrrrr');
...
const getShopPage = async (req, res) => {
  try {
      ...
      if (sort === "highToLow") {
          ...
          currentProduct = await Product.find({ isBlocked: false }).sort({ createdOn: 1 });
      } else {
...
const filterProduct = async (req, res) => {
  try {
      const categoryId = req.query.category; // Extract the category from the query parameters
...
const cancelOrder = async (req, res) => {
  try {
      ...
      // Check if the order status is confirmed or pending
      if (order.orderStatus === 'confirmed' || order.orderStatus === 'pending') {
          // Update the order status to 'cancelled'
          order.orderStatus = 'cancelled';
          await order.save();
          // Return the products' quantities back to the database
          for (const item of order.orderedItems) {
            ...
            if (!product.productSizes || !Array.isArray(product.productSizes)) {
              ...
              continue; // Skip to the next iteration if product sizes are not defined or not an array
            }
            const sizeIndex = product.productSizes.findIndex(size => size.size === item.size);
            if (sizeIndex !== -1) {
              // Increment the quantity of the cancelled size
          ...
          // If the order status is not confirmed or pending, it cannot be cancelled
